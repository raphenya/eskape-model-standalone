...
def run_random_forest_models(test_path, checkpoint_dir, preds_path):
    ...
    This function runs the random forest models. 
    The predictions are saved in a CSV file with the name "{preds_path}-rf.csv".
...
def run_chemprop_models(test_path, checkpoint_dir, preds_path):
    ...
    This function runs the chemprop models using the default features. 
    The predictions are saved in a CSV file with the name "{preds_path}-chemprop.csv".
...
def run_chemprop_rdkit_models(test_path, checkpoint_dir, preds_path):
    ...
    This function runs the chemprop models using rdkit features. The predictions are saved in a CSV 
    file with the name "{preds_path}-rdkit.csv". The rdkit features are generated using the 
    "rdkit_2d_normalized" features generator, and no feature scaling is applied.
...
def get_canonical_data(f1, f2, f3):
    ...
    This function takes in the path to the canonical training data, the path to the canonical results 
    file generated by the chemfunc canonicalize_smiles function, and the path where the validated 
    results file will be saved. It reads in the two csv files, merges them on the "smiles" column, 
    and saves the merged file as a new csv file with the name "{f3}-validated.txt". The merged file 
    contains all the columns from both input files, and is used to validate the predictions made by 
    the chemprop and random forest models against the training data.
...
def predict_function(args):
    ...
    This function is the main function that runs the ESKAPE model.
...
def launch_job(test_path, base_directory, preds_path, pathogen):
    ...
    This function runs the chemprop, rdkit, and random forest models for the given pathogen. 
    The predictions are saved in separate CSV files for each model type.
...
def worker(test_path, base_directory, preds_path):
    ...
    This function launches the prediction jobs for each pathogen.
...
def get_properties(o_f_path, accession, smile):
    ...
    This function takes in a smile and the path to the properties results file, and returns a 
    dictionary with the molecular weight and clogP for that smile.
...
def get_nearest_neighbor(o_f_path, accession, smile):
    ...
    This function takes in a smile and the path to the nearest neighbor results file, and returns 
    a dictionary with the Tanimoto nearest neighbor and its similarity for that smile.
...
def get_validated(o_f_path, accession, smile):
    ...
    This function takes in a smile and the path to the validated results file, and returns a 
    dictionary with the validated scores for each property (EF, SA, KP, AB, PA, BW, DKO) for that smile. 
...
def get_sum_ppfs(smile, j):
    ...
    This function takes in a smile and the json file generated by the parse_results function, and 
    returns a dictionary with the sum of the prediction scores for each model type (rdkit, chemprop, rf) 
    and the ppf (predicted probability of being active) for each model type.
...
def filter_results_tabular(filename=False, cplog=False, molweight=False, sum=False, ppf=False, modeltyperdkit=False, modeltypechemprop=False, modeltyperf=False, validated=False, tnn=False):
    ...
    This function reads the tabular results file generated by the generate_results_tabular function 
    and filters out columns based on the input parameters. The filtered results are then saved in a 
    new tabular file.
...
def generate_results_tabular(o_f_path, accession):
    """
    This function reads the json file generated by the parse_results function and creates a tabular results file.
    """
...
def parse_results(path):
    ...
    This function reads the csv files generated by the chemprop and random forest models, as well as 
    the properties and nearest neighbor files generated by the chemfunc package, and creates a json 
    file with all the results. The json file is then used to generate a tabular tsv file with all the 
